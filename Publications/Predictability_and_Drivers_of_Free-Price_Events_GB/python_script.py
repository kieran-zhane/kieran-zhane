...
# -*- coding: utf-8 -*-
"""
Paper Title: Predictability and Drivers of Free-Price Events in Great Britain’s Electricity Market: A 2024 Price-Carbon Analysis
Author: Kieran Zhané
"""
...
def plot_price_vs_ci_scatter(df: pd.DataFrame):
    ...
    # Interpolate LOWESS curve back onto each x_sorted to get pointwise predictions
    y_hat = np.interp(x_sorted, x_smooth, y_smooth)
    
    from sklearn.metrics import mean_squared_error, mean_absolute_error
    rmse = np.sqrt(mean_squared_error(y_sorted, y_hat))
    mae  = mean_absolute_error(y_sorted, y_hat)
